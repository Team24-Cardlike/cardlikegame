...
public class GameMap {
    ...
    GameMap(float hpModifier, float dmgModifier, GameManager manager, MapObserver mapObs){
        this.map  = new Graph<>();
    ...
    public void levelSelect(String s) {    
        ...
        for (int i = 0; i < this.regularOpponents.size(); i++) {
            ...
            if (s == op.getName()) System.out.println(this.map.neighbours(op) + " " + s);
            System.out.println(this.availableLvls + " " + this.completedLvls);
            if (s == op.getName() && this.availableLvls.contains(op.getName()) && !this.completedLvls.contains(op.getName())) {
